...
export const meta = {
  ...
  author: 'Oscar Newman',
  description:
    'A better solution for client-side environment variables in Remix',
  published: false,
}
...
`process.env.NEXT_PUBLIC_[your var]`.
...
export function getBuildEnvVars() {
  return Object.entries(process.env).filter(([key]) => key.startsWith('BUILD_'))
}
```
